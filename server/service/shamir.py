...
def split_private_key_for_shamir_phe(key: str) -> list[int]:
    """
    PHE用の秘密鍵分割
    """
    if len(key) != 64:
    ...
    key_part1 = int(key[:32], 16)
    key_part2 = int(key[32:], 16)
...
def generate_shamir_keys_phe(key: str) -> list[tuple[int, int, str]]:
    """
    PHE用のシャミアの秘密分散法を使って秘密鍵を分割する
    """
    # 秘密鍵を分割
    splited_key = split_private_key_for_shamir_phe(key)
    # シャミアの秘密分散法を使って秘密鍵を分割
    shamirs = [Shamir.split(3, 5, k.to_bytes((k.bit_length() + 7) // 8, 'big'), None) for k in splited_key]
...
def decrypt_shares(encrypted_shares: list[tuple[int, int, str]]) -> list[tuple[int, int, str]]:
    ...
    for i, idx, s in encrypted_shares:
        ...
        decrypted_shares.append((i, idx, str(decrypted_s)))  # 文字列として保存
        logger.debug(f"Decrypted share: (i={i}, idx={idx}, s={str(decrypted_s)})")
...
def recover_shamir_keys_phe(shares: list[tuple[int, int, str]]) -> str:
    """
    PHE用のシェアから復元する
    """
    s1 = []
    ...
    for i, idx, s in shares:
        logger.debug(f"Recovering share: {i}, {idx}, {s}")
        if i == 0:
            s1.append((idx, int(s).to_bytes((int(s).bit_length() + 7) // 8, 'big')))
            continue

        if i == 1:
            s2.append((idx, int(s).to_bytes((int(s).bit_length() + 7) // 8, 'big')))
            continue
